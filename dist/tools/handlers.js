...
import { updateBrowserState, validateArgs } from "../utils/helpers.js";
// Helper functions for browser state validation
export function validateBrowserState(logger, operation, requiresActivePage = true) {
...
export function validateMonitoringState(logger, operation, requiresActive = true) {
    const state = logger.getSessionState();
...
// Retry logic wrapper
function withRetry(operation, operationName, logger, config = DEFAULT_RETRY_CONFIG) {
    let lastError;
    return (async () => {
        for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
            try {
                ...
                if (attempt === config.maxAttempts) {
                    ...
                    throw new AgentFriendlyError("OPERATION_FAILED", `${operationName} failed after ${attempt} attempts: ${lastError.message}`, `Operation failed consistently. Check logs for details.`, false);
                }
                const delay = config.initialDelay * Math.pow(config.backoffMultiplier, attempt - 1);
                logger.debug(`Retry ${attempt}/${config.maxAttempts} for ${operationName} after ${delay}ms`);
...
export async function handleLaunchBrowser(args, logger, context) {
    return withRetry(async () => {
        ...
        if (page) {
            ...
            context.server.setBackendMocker?.(backendMocker);
        }
        return result;
    }, "launch_browser", logger);
}
export async function handleCloseBrowser(args, logger) {
    const result = await browserManager.closeBrowser();
    updateBrowserState(logger, false, false, false); // Reset all states
    return result;
}
// ============ ELEMENT INTERACTION HANDLERS ============
export async function handleFindElement(args, context) {
    if (!context.elementLocator) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
    const element = await context.elementLocator.findElement(args);
    return {
        content: [
            {
                type: "text",
...
export async function handleFillForm(args, context) {
    ...
    await context.formHandler.fillForm(args.fields);
    return {
        content: [
            {
                type: "text",
...
export async function handleSubmitForm(args, context) {
    if (!context.formHandler) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
...
export async function handleClickElement(args) {
...
export async function handleGetElementText(args) {
    return await uiInteractions.getElementText(args);
}
// ============ VISUAL TESTING HANDLERS ============
export async function handleTakeElementScreenshot(args, logger) {
    const page = browserManager.getPage();
    if (!page) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
    ...
    const elementScreenshot = await visualTesting.takeElementScreenshot(page, args.selector, {
        format: args.format,
        quality: args.quality,
        padding: args.padding,
    });
    const elementPath = path.join(process.cwd(), DIRECTORIES.screenshots, "current", `${args.name}.png`);
...
export async function handleTakeResponsiveScreenshots(args) {
    const page = browserManager.getPage();
    if (!page) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
    ...
    const breakpoints = Array.isArray(args.breakpoints)
        ? args.breakpoints
        : [320, 768, 1024, 1440];
    const responsiveScreenshots = await visualTesting.takeResponsiveScreenshots(page, breakpoints, {
        selector: args.selector,
        fullPage: args.fullPage,
    });
    const responsiveResults = Array.from(responsiveScreenshots.entries()).map(([width, buffer]) => {
        const responsivePath = path.join(process.cwd(), DIRECTORIES.screenshots, "current", `${args.name}_${width}px.png`);
...
export async function handleGetJourneyScreenshots(args) {
    ...
    try {
        ...
        for (const subDir of possibleDirs) {
            ...
            if (fs.existsSync(fullSubDir)) {
                await scanDirectoryForScreenshots(fullSubDir, subDir, journeyScreenshots, journeyFilter);
            }
            ...
            if (fs.existsSync(journeySubDir)) {
                ...
                for (const journeyFile of journeyFiles) {
                    ...
                    if (fs.statSync(journeyPath).isDirectory()) {
                        await scanDirectoryForScreenshots(journeyPath, `${subDir}/journeys/${journeyFile}`, journeyScreenshots, journeyFilter);
                    }
                }
            }
        }
        ...
        const groupedScreenshots = journeyScreenshots.reduce((acc, shot) => {
        ...
        let summary = "";
        if (journeyFilter) {
            const filteredShots = journeyScreenshots.filter((s) => s.journeyName && s.journeyName.toLowerCase().includes(journeyFilter));
        ...
        Object.keys(groupedScreenshots).forEach((groupKey) => {
            ...
            const totalSize = group.reduce((sum, shot) => sum + shot.size, 0);
            ...
            if (journeyFilter && groupKey.toLowerCase().includes(journeyFilter)) {
                ...
                summary += `📂 ${groupKey.replace("_", "-")}: ${group.length} screenshots (${formatBytes(totalSize)})\n`;
            }
        });
        // Detailed list with filtering
        const filteredScreenshots = journeyFilter
            ? journeyScreenshots.filter((shot) => shot.journeyName &&
    ...
    📍 Path: ${shot.path}`;
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Failed to retrieve journey screenshots: ${error.message}`,
                },
            ],
        };
    }
...
async function scanDirectoryForScreenshots(directory, category, screenshots, journeyFilter) {
    try {
                ...
                try {
                    ...
                    if (directory.includes("journey") || file.includes("journey")) {
                        if (file.match(/journey[_-]([a-zA-Z_-]+)/)) {
                            ...
                            const journeyIndex = dirParts.findIndex((part) => part.includes("journey"));
...
export async function handleLoadMockConfig(args, context) {
...
export async function handleSaveMockConfig(args, context) {
    if (!context.backendMocker) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
    if (!args || typeof args.name !== "string") {
...
export async function handleAddMockRule(args, context) {
    if (!context.backendMocker) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
...
export async function handleRemoveMockRule(args, context) {
    if (!context.backendMocker) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
...
export async function handleUpdateMockRule(args, context) {
    if (!context.backendMocker) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
...
export async function handleGetMockRules(context) {
    if (!context.backendMocker) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
...
export async function handleEnableBackendMocking(context) {
    if (!context.backendMocker) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
    const page = browserManager.getPage();
...
export async function handleDisableBackendMocking(context) {
    if (!context.backendMocker) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
    const page = browserManager.getPage();
...
export async function handleGetMockedRequests(context) {
    if (!context.backendMocker) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
...
export async function handleClearAllMocks(context) {
    if (!context.backendMocker) {
        throw new Error("Browser not launched. Please launch browser first.");
    }
...
export async function handleSetupJourneyMocks(args, context) {
...
export async function handleToolRequest(name, args, context) {
    ...
    switch (name) {
        ...
        case "detect_visual_regression":
            return await visualTesting.compareWithBaseline(browserManager.getPage(), args.testName, args);
        case "update_baseline":
            await visualTesting.updateBaseline(browserManager.getPage(), args.testName);
            return {
                content: [
                    {
                        type: "text",
