...
export async function handleGetServerState(server, args) {
...
- Active Tools: ${sessionState.activeTools.length > 0
                    ? sessionState.activeTools.join(", ")
                    : "None"}
- Last Activity: ${sessionState.lastActivity.toISOString()}
- Session Started: ${new Date(Date.now() -
                    (sessionState.lastActivity.getTime() - new Date().getTime())).toISOString()}`,
            },
        ],
    };
}
export async function handleGetSessionInfo(server, args) {
...
- Active Tools: ${currentState.activeTools.length > 0
