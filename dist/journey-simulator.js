import { createJourneyVideoRecorder } from "./video-recorder.js";
export class JourneySimulator {
    ...
    async runJourney(options) {
        ...
        if (options.recording?.enabled) {
            try {
                this.videoRecorder = createJourneyVideoRecorder(this.page, options.name);
                recordingSessionId = await this.videoRecorder.startRecording();
                console.log(`🎥 Started recording journey "${options.name}" with session ID: ${recordingSessionId}`);
            }
            catch (error) {
        ...
        try {
            ...
            if (this.videoRecorder && recordingSessionId) {
                try {
                    ...
                    console.log(`🎥 Stopped recording journey "${options.name}". Recorded ${metadata.actions.length} actions.`);
                }
                catch (error) {
    ...
    async executeStep(step) {
        ...
        const shouldCaptureAction = this.currentJourney?.recording?.enabled &&
