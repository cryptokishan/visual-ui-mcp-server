...
export class JourneyRecorder {
     ...
     */
    async setupEventListeners(page, session) {
        // Node-side click handler: receives ElementHandle
        const clickListener = async (elementHandle) => {
            if (!session.isRecording || !this.shouldRecordInteraction())
                return;
            if (!elementHandle)
                return;
            const selector = await this.generateSelectorFromHandle(page, elementHandle);
        ...
        const inputListener = async (elementHandle) => {
            if (!session.isRecording || !this.shouldRecordInteraction())
                return;
            if (!elementHandle)
                return;
            // Skip password fields
            const type = await elementHandle.evaluate((el) => el.type);
            if (type === "password")
                return;
            const selector = await this.generateSelectorFromHandle(page, elementHandle);
        ...
        await page.exposeBinding("__journey_click", async (source, elementHandle) => {
            await clickListener(elementHandle);
        }, { handle: true });
        await page.exposeBinding("__journey_input", async (source, elementHandle) => {
        ...
        await page.addInitScript(() => {
            const attachListeners = () => {
                window.addEventListener("click", (event) => {
                    ...
                    if (target && typeof window["__journey_click"] === "function") {
                        window["__journey_click"](target);
                    }
                }, true);
                ...
                document.addEventListener("input", (event) => {
                    ...
                    if (target &&
                        ((target.tagName === "TEXTAREA") ||
                            (target.tagName === "INPUT" && inputTypes.includes(target.type))) &&
                        typeof window["__journey_input"] === "function") {
                        window["__journey_input"](target);
                    }
                }, true);
            };
            window.__journey_attachListeners = attachListeners;
            attachListeners();
        });
        await page.evaluate(() => {
            window.__journey_attachListeners?.();
        });
    ...
    async generateSelectorFromHandle(page, elementHandle) {
    ...
    async describeClickFromHandle(elementHandle) {
        return await elementHandle.evaluate((el) => {
    ...
    async describeInputFromHandle(elementHandle) {
        return await elementHandle.evaluate((el) => {
