...
export async function handleMeasureCoreWebVitals(server: any, args: any) {
  ...
  const coreWebVitals =
    await server.performanceMonitor.measureCoreWebVitals(pageForVitals);
...
export async function handleAnalyzePageLoad(server: any, args: any) {
  ...
  server.performanceMonitor = new PerformanceMonitor();
  const loadAnalysis = await server.performanceMonitor.analyzePageLoad(
    pageForLoad
  );
...
- Resource Types: ${[
          ...new Set(
            loadAnalysis.resourceTiming.map((r: any) => r.initiatorType)
          ),
        ].join(", ")}`,
      },
    ],
...
export async function handleMonitorResourceLoading(server: any, args: any) {
  ...
  const resourceTiming =
    await server.performanceMonitor.monitorResourceLoading(pageForResources);
...
export async function handleTrackMemoryUsage(server: any, args: any) {
  ...
  const memoryHistory =
    await server.performanceMonitor.trackMemoryUsage(pageForMemory, duration);
...
export async function handleDetectPerformanceRegression(server: any, args: any) {
  ...
  const currentMetrics =
    await server.performanceMonitor.getComprehensiveMetrics(pageForRegression);
  const regressionReport =
    await server.performanceMonitor.detectPerformanceRegression(
...
export async function handleGetComprehensivePerformanceMetrics(server: any, args: any) {
  ...
  const comprehensiveMetrics =
    await server.performanceMonitor.getComprehensiveMetrics(
