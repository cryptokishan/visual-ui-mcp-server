...
export class DevToolsMonitor {
  ...
  async getConsoleLogs(args: any) {
    try {
      ...
      throw new Error(`Failed to get console logs: ${(error as Error).message}`);
    }
  }

  async getNetworkRequests(args: any) {
    try {
      ...
      if (filter) {
        filteredRequests = this.networkRequests.filter(req =>
        ...
        content: [
          {
            ...
            text: `Network requests (${result.length}):\n${result.map(req =>
              `${req.method} ${req.url} - ${req.status || 'Pending'} (${req.duration || 0}ms)${req.failed ? ' [FAILED]' : ''}`
            ).join('\n')}`,
          },
        ],
      };
    } catch (error) {
      throw new Error(`Failed to get network requests: ${(error as Error).message}`);
    }
  }

  async checkForErrors(args: any) {
    try {
      ...
      if (includeConsoleErrors) {
        const consoleErrors = this.consoleMessages.filter(msg =>
          msg.type === 'error'
        );
      ...
      if (includeNetworkErrors) {
        const networkErrors = this.networkRequests.filter(req =>
          req.failed || (req.status && req.status >= 400)
        );
      ...
      throw new Error(`Failed to check for errors: ${(error as Error).message}`);
    }
  }
