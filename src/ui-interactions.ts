...
export class UIInteractions {
  ...
  async typeText(args: any) {
    try {
      ...
      throw new Error(`Failed to type text: ${(error as Error).message}`);
    }
  }

  async getElementText(args: any) {
    try {
  ...
  async getElementAttribute(args: any) {
    try {
      ...
      throw new Error(
        `Failed to get element attribute: ${(error as Error).message}`
      );
    }
  }

  async isElementVisible(args: any) {
    try {
      const { selector, selectorType = "css" } = args;
      const page = browserManager.getPage();
      ...
      throw new Error(
        `Failed to check element visibility: ${(error as Error).message}`
      );
    }
  }

  async scrollToElement(args: any) {
    try {
      const { selector, selectorType = "css" } = args;
      const page = browserManager.getPage();
      ...
      throw new Error(`Failed to scroll to element: ${(error as Error).message}`);
    }
  }
}
