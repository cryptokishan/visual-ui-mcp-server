...
export class WaitRetrySystem {
  ...
  async waitForCondition(args: any) {
    try {
      const {
        condition,
        timeout = 10000,
      ...
      throw new Error(`Failed to wait for condition: ${(error as Error).message}`);
    }
  }

  async waitForText(args: any) {
    try {
      const {
        selector,
        text,
      ...
      throw new Error(`Failed to wait for text: ${(error as Error).message}`);
    }
  }

  async waitForURL(args: any) {
    try {
      const {
        url,
        timeout = 10000,
  ...
  async waitForNetworkIdle(args: any) {
    try {
      const {
        timeout = 30000,
        concurrent = 2
      ...
      throw new Error(`Failed to wait for network idle: ${(error as Error).message}`);
    }
  }

  async retryAction(args: any) {
    try {
      const {
        action,
        condition,
      ...
      throw new Error(`Failed to retry action: ${(error as Error).message}`);
    }
  }
}
